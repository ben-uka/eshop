...
public class SupplierRepository(DataContext context, IAddressRepository repo) : ISupplierRepository
{
    ...
    public async Task<bool> Add(SupplierPostViewModel model)
    {
        ...
        {
            if (
                await _context.Suppliers.FirstOrDefaultAsync(c =>
            ...
            foreach (var a in model.Addresses)
            {
                ...
                await _context.SupplierAddresses.AddAsync(
                    new SupplierAddress { Address = address, Supplier = supplier }
                );
            }
    ...
    public async Task<IList<SuppliersViewModel>> List()
    {
        ...
        {
            throw new Exception(
                $"Ett fel inträffade i ListAllSupplier(SupplierRepository) {ex.Message}"
            );
        }
    }
