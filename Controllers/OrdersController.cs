...
public class OrdersController(DataContext context) : ControllerBase
{
    ...
    public async Task<ActionResult> ListAllOrders()
    {
        ...
        var orders = await _context
            .SalesOrders.Include(c => c.OrderItems) // Vi måste nu projicera datat till det format som vi vill leverera tillbaka...
            .Select(order => new
            {
    ...
    public async Task<ActionResult> FindOrder(int id)
    {
        var order = await _context
            .SalesOrders.Where(o => o.SalesOrderId == id)
        ...
        if (order is null)
        {
            return NotFound(
                new
                {
                    success = false,
                    statusCode = 404,
                    message = $"Tyvärr vi kunde inte hitta någon beställning med ordernummer: {id}",
                }
            );
        }
        return Ok(
    ...
    {
        var newOrder = new SalesOrder { OrderDate = order.OrderDate, OrderItems = [] };
        ...
        foreach (var product in order.Products)
        {
            ...
            if (prod is null)
                return BadRequest($"Du har angivet ett produkt id som inte existerar");
        ...
        try
        {
            ...
            return CreatedAtAction(
                nameof(FindOrder),
                new { id = newOrder.SalesOrderId },
                new { newOrder.SalesOrderId, newOrder.OrderDate }
            );
        }
        catch (Exception ex)
        {
    ...
    {
        var orderToUpdate = await _context
            .SalesOrders.Where(c => c.SalesOrderId == id)
            .Include(o => o.OrderItems)
            .SingleOrDefaultAsync();

        if (orderToUpdate is null)
            return BadRequest($"Det finns ingen beställning med ordernummer {id}");
